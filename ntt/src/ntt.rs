...
pub fn transpose<T: Copy>(matrix: &[T], rows: usize, columns: usize) -> Vec<T> {
...
pub fn transpose_tiled<T: Copy + Send + Sync>(matrix: &[T], rows: usize, columns: usize) -> Vec<T> {
    use rayon::prelude::*;
...
mod tests {
    ...
    use {
        super::{init_roots_reverse_ordered, reverse_order, transpose, transpose_tiled},
    ...
    proptest! {
        ...
        #[test]
        fn test_transpose_tiled((rows, columns, mat) in (256_usize..320, 256_usize..320)
            .prop_flat_map(|(rows, columns)| {
                // If either rows or columns is zero, the matrix is empty
                let len = rows * columns;
                (Just(rows), Just(columns), collection::vec(any::<u32>(), len..=len))
            ...
            let transposed = transpose_tiled(&mat, rows, columns);
            let double_transposed = transpose_tiled(&transposed, columns, rows);
            assert_eq!(mat, double_transposed);
        }
    }
